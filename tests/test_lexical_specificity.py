""" UnitTest for lexical specificity """
import unittest
import logging
from logging.config import dictConfig

from grapher import LexicalSpec
from grapher.lexical_specificity import lexical_specificity
...
class TestLexicalSpecificity(unittest.TestCase):
    """Test lexical_specificity """

    def test_lexical_specificity(self):
        score = lexical_specificity(T=10000, t=500, f=20, k=6)
        score = round(score, 5)
        assert 3.49193 == score, "{} not matched to 3.49193".format(score)

    def test_keyword_extraction(self):
        model = LexicalSpec()
        model.train(TEST_DOCS, export_directory='./tests')
