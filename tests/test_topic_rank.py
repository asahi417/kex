...
class TestTopicRank(unittest.TestCase):
    ...
    def test_topic_rank(self):
        out = TopicRank('en').get_keywords(test_en)
        for i in out:
            LOGGER.info(i)
        out = TopicRank('ja').get_keywords(test_ja)
        for i in out:
            LOGGER.info(i)

    def test_multi_rank(self):
        out = MultipartiteRank(language='en').get_keywords(test_en)
        for i in out:
            LOGGER.info(i)
        out = MultipartiteRank(language='ja').get_keywords(test_ja)
        for i in out:
            LOGGER.info(i)
