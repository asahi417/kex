...
class StopwordDetection:
    ...
    def apply(self,
        ...
        if string in self.exclude:
            return False

        for _rule in self.regx_full_list_token:
            if re.search(_rule, string.lower()):
                if debug:
                    print(_rule)
                return True
        return False
...
class SkipPhrase:
    ...
    def __init__(self, new_skip_phrase: list = None):
        self.__reg_phrase = [
    ...
    def apply(self, string, debug: bool = False):
        for _rule in self.__reg_phrase:
            if re.search(_rule, string):
                if debug:
                    print(_rule)
                return True
        return False
